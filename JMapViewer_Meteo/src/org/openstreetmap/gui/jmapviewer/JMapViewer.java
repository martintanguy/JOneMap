...
public class JMapViewer extends JPanel implements TileLoaderListener {
	...
	private Double displayedBoundingBox;
...
//	private TileController tc5;
//
//	private TileController tc6;
//
//	private TileController tc7;
//
//	private TileController tc8;

    /**
     * Creates a standard {@link BackUpJMapViewer} instance that can be controlled via
...
//        int x = center.x + getPosition().mapPointX - getWidth() / 2;
...
//        double lat = tileSource.YToLat(y, zoom);
		...
		tc1 = new TileController(tileSource, tileCache, this);
		tc2 = new TileController(tileSource, tileCache, this);
		tc3 = new TileController(tileSource, tileCache, this);
		tc4 = new TileController(tileSource, tileCache, this);
		
//		new VectorController()
		
//		tc5 = new TileController(tileSource, tileCache, this);
//		tc6 = new TileController(tileSource, tileCache, this);
//		tc7 = new TileController(tileSource, tileCache, this);
//		tc8 = new TileController(tileSource, tileCache, this);

		tileControllers.add(tc1);
		tileControllers.add(tc2);
		tileControllers.add(tc3);
		tileControllers.add(tc4);
//		tileControllers.add(tc5);
//		tileControllers.add(tc6);
//		tileControllers.add(tc7);
//		tileControllers.add(tc8);
...
//        lat = tileSource.XToLon(aX, zoom),
//        for(MeteoCoordinate c:New_Test.grib(8, Up_x, Up_y, Down_x, Down_y))
//         addMapMarker(new BoatMarker (c));
        
        
//        
//        tc5.tileSource = new GeomodTileSources.GSoundings();
//        tc6.tileSource = new GeomodTileSources.GRestriction();
//        tc7.tileSource = new GeomodTileSources.GDangers();
//        tc8.tileSource = new GeomodTileSources.GMetadata();
...
//        tileController = new TileController(tileSource, tileCache, this);
        ...
        setDisplayPosition(new Coordinate(48.3262,-4.4256), 8);
    ...
    protected void initializeZoomSlider() {
        zoomSlider = new JSlider(MIN_ZOOM, tc1.getTileSource().getMaxZoom());
    //    zoomSlider.setOrientation(JSlider.VERTICAL);
        zoomSlider.setBounds(18000, 10, 30, 150);
        ...
        zoomInButton.setBounds(400, 5, size, size);
        ...
        zoomOutButton.setBounds(4008, 155, size, size);
    ...
    public void setDisplayPosition(Point mapPoint, int x, int y, int zoom) {
        if (zoom > tc1.getTileSource().getMaxZoom() || zoom < MIN_ZOOM)
            return;
...
//                            	tile.paintT(g, posx, posy); 
        ...
        if (mapMarkersVisible && mapMarkerList != null) {
        	getDisplayBoundingBox();
            for (MapMarker marker : mapMarkerList) {
                if(marker.isVisible()&& displayedBoundingBox.contains(marker.getLon(), marker.getLat()))
        ...
        displayedBoundingBox = null;
        Up_x = getPosition(0, 0).getLat();
        Up_y = getPosition(0, 0).getLon();
        Down_x = getPosition(getWidth(), getHeight()).getLat();
        Down_y = getPosition(getWidth(), getHeight()).getLon();
        attribution.paintAttribution(g, getWidth(), getHeight(), getPosition(0, 0), getPosition(getWidth(), getHeight()), zoom, this);
	...
	private void getDisplayBoundingBox() {
		if (displayedBoundingBox != null)
			return;
		ICoordinate topLeft = getPosition(0, 0);
		ICoordinate bottomRight = getPosition(getWidth(), getHeight());
		displayedBoundingBox = new Rectangle2D.Double(topLeft.getLon(), bottomRight.getLat(),
				bottomRight.getLon() - topLeft.getLon(), topLeft.getLat() - bottomRight.getLat());
	}
    ...
    public void setZoom(int zoom, Point mapPoint) {
        if (zoom > tc1.getTileSource().getMaxZoom() || zoom < tc1.getTileSource().getMinZoom()
                || zoom == this.zoom)
            return;
...
//    		System.out.println("NO POINT DELECTED");}}
    ...
    public TileController getTileController(int index) {
        return tileControllers.get(index);
    }
	...
	public void globalSet() {
		...
		tc1.setTileSource(tc1.tileSource);
		tc2.setTileSource(tc2.tileSource);
		tc3.setTileSource(tc3.tileSource);
		tc4.setTileSource(tc4.tileSource);
		this.fireJMVEvent(new JMVCommandEvent(COMMAND.MOVE, this));

		
	}
	
	public void DisplayTransparency1(boolean enabled) {
		if (tc1t == true) {
			tc1t = false; repaint();} 
		else { tc1t = true; repaint(); }
			;
	}
	
	public void DisplayTransparency2(boolean enabled) {
		if (tc2t == true) {
			tc2t = false; repaint();} 
		else { tc2t = true; repaint();}
			;
	}
	
	public void DisplayTransparency3(boolean enabled) {
		if (tc3t == true) {
			tc3t = false; repaint();} 
		else { tc3t = true; repaint(); }
			;
	}
	
	public void DisplayTransparency4(boolean enabled) {
		if (tc4t == true) {
			tc4t = false; repaint();} 
		else { tc4t = true; repaint(); }
			;
	}
